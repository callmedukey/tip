...
const MainPageForm = ({
  ...
  const onSubmit = async (data: z.infer<typeof MainPageFormSchema>) => {
    if (!data.city || !data.city.length) {
      setError(true);
      setErrorMessage(t("noCity"));
      return;
    }

    if (!date.from) {
      setError(true);
      setErrorMessage(t("noDates"));
      return;
    }
    ...
    if (!data.adults) {
      setError(true);
      setErrorMessage(t("noPersons"));
      return;
    }

    if (!data.purpose) {
      setError(true);
      setErrorMessage(t("noPurpose"));
      return;
    }
  ...
  return (
    <>
      ...
      <Dialog open={error} onOpenChange={setError}>
        <DialogContent className="lg:top-[calc(var(--header-height)+7rem)] min-h-[10rem] flex flex-col justify-center items-center max-w-sm sm:rounded-3xl rounded-3xl">
          <DialogHeader>
            <DialogTitle className="text-black font-medium font-pretendard">
              {errorMessage}
            </DialogTitle>
