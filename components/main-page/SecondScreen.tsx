...
const SecondScreen = ({
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-center lg:flex-row flex-col max-w-screen-8xl mx-auto w-full text-white gap-4 lg:mt-16 mt-24 px-4 pb-4 pt-4 lg:py-0 self-start lg:self-center">
        ...
        <div className="flex flex-col lg:block items-end justify-end relative basis-full -mt-16 lg:mt-0 lg:basis-1/2 flex-1 shrink-0 min-h-[45rem] xl:min-h-[calc(100vh-var(--header-height)-20rem)] isolate w-full lg:rounded-none rounded-[3rem] overflow-clip">
          ...
          <div className="lg:absolute z-10 py-24 rounded-[3rem] lg:rounded-none lg:top-0 lg:right-0 lg:max-w-[45%] -mt-16 lg:mt-0 w-full h-full bg-[#036DBA] lg:bg-[#036DBA]/80 flex flex-col gap-4 items-center justify-center text-center">
            <h2 className="font-bold text-[1.25rem]/[100%]">Custom package</h2>
            <p className="lg:max-w-[14rem] max-w-sm">
              You create and we assist planning a package based on your specific
              needs
            </p>
            <Button
      ...
      <button
        className="absolute bottom-4 py-4 max-w-xs w-full mx-auto bg-white/50 mt-12 rounded-full font-medium text-cancel border border-cancel hover:bg-white hidden lg:block"
        type="button"
        onClick={handleReturnToFirstScreen}
      >
        Cancel
