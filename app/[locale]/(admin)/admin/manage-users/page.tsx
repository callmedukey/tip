...
if (!session || !session.userId || session.accountType!=='Admin'){return redirect ("/login") }
  ...
  }, select:{name:true, id:true, email:true, moneySpent:true, userLevel:true, gender:true, birthday:true },
      ...
      <Table className='text-black'>
        ...
        <TableBody>
          {users.map ((user)=>(
            ...
            <TableCell>{user.moneySpent}</TableCell>
            <TableCell >{userLevelObject[user.userLevel]}</TableCell>
            <TableCell>{user.gender} {formatDateToKR(user.birthday)} </TableCell>
            <TableCell><Link href={`/admin/manage-users/${user.id}`}><SquareArrowOutUpRight /></Link></TableCell>
          </TableRow>
))}
        </TableBody>
      </Table>
